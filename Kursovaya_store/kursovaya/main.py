...
class Form_Sotrydniki(QWidget, Ui_Sotrydniki): #таблица вакансия
   ...
   def insert_Sotrydniki(self): #кнопка добавить
       row = [self.lineName.text(), self.lineFamilia.text(), self.cbPost.itemText(self.cbPost.currentIndex()), self.lineDate.text(), self.linePrice.text(), 'м' if self.rbMan.isChecked() else 'ж', self.lineSerias.text(), self.lineNamber.text()]
       try:
           cur = self.conn.cursor()
           cur.execute(f"""insert into Sotrydniki(Name, Familia, Dolzenost, Data_priema_na_raboty, Price, Pol, Seria, Nomer)
   ...
   def update_record(self):#изменение
       ...
       try:
           cursor.execute(
               """UPDATE Sotrydniki SET Name=?, Familia=?, Dolzenost=?, Data_priema_na_raboty=?, Price=?, Pol=?, Seria=?, Nomer=? WHERE ID_sotrydniki=?""",
...
class Form_Category(QWidget, Ui_Category_tovarov): #таблица вакансия
   ...
   def update_Category(self):#изменение
       ...
       try:
           cursor.execute(
               """UPDATE Interview SET Naimenovanie=? WHERE ID_category=?""",
               (Naimenovanie))
           conn.commit()
       except Exception as e:
